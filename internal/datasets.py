...
def load_cnf_from_dimacs(dimacs_file: str = None) -> dict:
  ...
  if dimacs_file is not None:
      ...
      for line in file:
        if (len(line) > 0):
          if line[0] == 'c': # begins with the character c is considered a comment.
            pass
          elif not seen_problem_statement and line[0] == 'p': # parse header line
            parsed_tokens = spilt_dimacs_line(line)
            if len(parsed_tokens) != 4:
              raise SyntaxError(f"invalid syntax: header line should be of the form p cnf <variables> <clause_list>, \
              ...
              try:
                ...
                try:
                  ...
                  print(f"Expect number of clause_list be integer, got {parsed_tokens[3][0]} at line number: {parsed_line_no}")
              except ValueError:
                print(f"Expect number of variables be integer, got {parsed_tokens[2][0]} at line number: {parsed_line_no}")
          else: # parse literals
            parsed_tokens = spilt_dimacs_line(line)
            for token, (col_start, col_end) in parsed_tokens:
              try:
                ...
                print(f"Expect literal be integer, got {token}, line no: {parsed_line_no}")
          parsed_line_no += 1
      file.close()
      # sanity check before return
      if num_vars < max_var:
        raise ValueError(f"the declared number of variables {num_vars} is smaller than the actual number of variables {max_var}") 

      if num_clauses != len(clause_list):
        raise ValueError(f"the declared number of clause_list {num_clauses} does not match the actual number of clause_list {len(clause_list)}")

  clauses_np = np.zeros(
    [len(clause_list), len(max(clause_list, key = lambda x: len(x)))], 
    dtype = np.int32
  )
        ...
        if (len(line) > 0):
          ...
          for val_str, (col_start, col_end) in parsed_string:
            try:
              ...
              print(f"Expect literal be integer, got {val_str}, line no: {parsed_line_no}")
          parsed_line_no += 1
      file.close()
...
class Dataset(threading.Thread):
  ...
  def constraint_params(self):
    """Dict contains necessary parameters for building solvable constaint"""
